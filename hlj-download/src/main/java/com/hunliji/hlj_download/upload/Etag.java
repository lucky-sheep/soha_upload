...
final class Etag {
     ...
     * @return 二进制数据的etag
     */
    private static String data(byte[] data, int offset, int length) {
        InputStream in = null;
        try {
     ...
     * @return 二进制数据的etag
     */
    private static String data(byte[] data) {
     ...
     */
    protected static String file(File file) throws IOException {
        InputStream in = null;
        try {
     ...
     * @throws IOException 文件读取异常
     */
    protected static String file(String filePath) throws IOException {
    ...
    private static String stream(InputStream in, long len) throws IOException {
        ...
        byte[][] blocks = new byte[(int) ((len + UploadConstant.BLOCK_SIZE - 1) / UploadConstant.BLOCK_SIZE)][];
        for (int i = 0; i < blocks.length; i++) {
            long left = len - UploadConstant.BLOCK_SIZE * i;
            long read = left > UploadConstant.BLOCK_SIZE ? UploadConstant.BLOCK_SIZE : left;
